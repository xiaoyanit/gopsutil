...
func test_getProcess() structs.Process {
	checkPid := os.Getpid()
	if runtime.GOOS == "windows" {
		checkPid = 7960
	}
	ret, _ := gopsutil.NewProcess(int(checkPid))
	return *ret
}
...
func Test_NewProcess(t *testing.T) {
	...
	ret, err := gopsutil.NewProcess(int(checkPid))
	if err != nil {
		t.Errorf("error %v", err)
	}
	empty := &structs.Process{}
