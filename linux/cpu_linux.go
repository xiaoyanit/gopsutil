...
func CPUTimes(percpu bool) ([]structs.CPUTimesStat, error) {
	...
	ret := make([]structs.CPUTimesStat, 0, len(lines))
...
func CPUInfo() ([]structs.CPUInfoStat, error) {
	...
	var ret []structs.CPUInfoStat

	var c structs.CPUInfoStat
	for _, line := range lines {
		...
		switch key {
		case "processor":
			c = structs.CPUInfoStat{}
			c.CPU = utils.MustParseInt32(value)
		case "vendor_id":
			c.VendorID = value
		case "cpu family":
			c.Family = value
		...
		case "model name":
			c.ModelName = value
		case "stepping":
			c.Stepping = utils.MustParseInt32(value)
		case "cpu MHz":
			c.Mhz = utils.MustParseFloat64(value)
		case "cache size":
			c.CacheSize = utils.MustParseInt32(strings.Replace(value, " KB", "", 1))
		case "physical id":
			c.PhysicalID = value
		case "core id":
			c.CoreID = value
		case "cpu cores":
			c.Cores = utils.MustParseInt32(value)
		case "flags":
...
func parseStatLine(line string) (*structs.CPUTimesStat, error) {
